...
These prompt recipes are used with `scripts/yunshu_imagegen.py` for both generation and editing.
...
The current Yunshu script accepts one `--image` input per edit request. For recipes that name multiple images, combine the references first or adapt the prompt to the primary input image.
...
Execution details such as the model, size, background mode, input image, and local output path are CLI arguments rather than prompt fields. Request transparent backgrounds with `--background transparent` and preserve the generated alpha.

Yunshu model notes:
- `gpt-image-2` is the default client-requested model for new workflows.
- Use `1024x1024` for square, `1536x1024` for landscape, or `1024x1536` for portrait unless the user requests another supported size.
- The upstream may return dimensions that differ slightly from the requested size; inspect the actual saved image.
