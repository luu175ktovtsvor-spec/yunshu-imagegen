...
These prompting principles are used with `scripts/yunshu_imagegen.py` for both generation and editing.

This file is about prompt structure, specificity, and iteration. Execution controls such as the model, size, background mode, and output path belong in CLI arguments rather than the image prompt.
...
- [Yunshu execution controls](#yunshu-execution-controls)
...
- The current Yunshu script accepts one `--image` input per edit request. If a recipe names multiple images, combine the references first or send the primary image and describe the remaining reference in the prompt.
- If the user provides an image for style, composition, or mood guidance and does not ask to modify it, treat the request as an edit that uses the image as a reference.
...
## Transparent images
- Pass `--background transparent` for a genuinely transparent background and preserve its alpha.

## Yunshu execution controls
- `--model`, `--size`, `--background`, `--image`, and `--output` are CLI arguments rather than prompt fields.
- The default client-requested model is `gpt-image-2`.
- Use `generate` for a new image and `edit --image /path/to/input.png` when a supplied image must be edited or included.
- Use `--background opaque` by default and `--background transparent` only when the user requests transparency.
