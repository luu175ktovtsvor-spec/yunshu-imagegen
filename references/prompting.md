...
These prompting principles follow Codex ImageGen and apply to Yunshu ImageGen agent requests.

This file is about prompt structure, specificity, invariants, and iteration. Request transport fields belong in [yunshu-interface.md](yunshu-interface.md), not inside the visual prompt unless they also describe the desired result.
...
- Treat examples in `sample-prompts.md` as fully-authored recipes, not as the default amount of augmentation to add to every user request.
...
- For small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes, keep the requested text concise and require verbatim rendering.
...
- If the user provides images only for style, composition, or mood guidance and does not ask to modify them, keep the intent as generation with references.
...
- Preserve image ordering and identify which image is the base, target, style reference, or supporting insert.
...
- Request a genuinely transparent background through the `background` field when the selected model supports it.
- Preserve and inspect the returned alpha channel.
- If the selected model cannot produce true transparency, explain the tradeoff before choosing a fallback.
...
- infographic-diagram: Define the audience and layout flow; label parts explicitly; require verbatim text; keep dense labels short and readable.
